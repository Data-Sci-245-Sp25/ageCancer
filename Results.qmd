...
group_cancer<-filtered_cancer_rates %>%
  ...
  group_by(diseaseSite, ageRanges)%>%
  summarize(averageRate = mean(rate_per_100k))
...
ggplot(group_cancer, aes(x = ageRanges, y = averageRate, color = diseaseSite)) +
  geom_point() +
...
cor_by_disease <- group_cancer %>%
  ...
  summarize(correlation = cor(ageRanges, averageRate, method = "pearson"))

print(cor_by_disease)

```
Based on a Pearson's correlation coefficient, there is a strong correlation between average rates of cancer and age. For colon cancer, the average rate of cancer and age has a correlation of 0.86. The correltation coefficient for the average liver cancer rate and age is 0.95.
